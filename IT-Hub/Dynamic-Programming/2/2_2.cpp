...
int recs(vector<int> &nums, unordered_map<int, int> &history, int start) {
  if (start > nums.size())
    return 0;
  int maxMoney = 0;
  for (int i = start; i < nums.size() && i < start + 2; i++) {
    if (!history.count(i))
      history[i] = recs(nums, history, i + 2);
    maxMoney = max(maxMoney, nums[i] + history.at(i));
  }
  return maxMoney;
}
